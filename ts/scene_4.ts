...
import {scene4Text} from "./textDescriptions";
...
export class scene4{
    ...
    private s4t:scene4Text = new scene4Text();
    ...
    private setupModel(){
        ...
        // Load in file.
        let meshTask = assetsManager.addMeshTask("scene4Model","","models/","scene4_model.babylon");
        // Mesh will be positioned, scaled then animation will be begin if loading is succesful.
        meshTask.onSuccess = function(task){
            task.loadedMeshes[0].name ="scene4Axe";
            task.loadedMeshes[0].id = "scene4Axe";
    ...
    private setupUI(){
        ...
        let label = new BABYLON.GUI.Rectangle("scene4Label");
        ...
        label.linkWithMesh(this.scene.getMeshByID("scene4model"));
        label.isVisible = false;

        let text1 = new BABYLON.GUI.TextBlock();
        text1.text = this.s4t.getMNameText();
        text1.color = "white";
        text1.textWrapping = true;
        label.addControl(text1); 

        let label2 = new BABYLON.GUI.Rectangle("Scene4Label");
        ...
        text2.text = this.s4t.getMDescText();
        text2.color = "white";
    ...
    private setupSound():void{
        let sound = new BABYLON.Sound("scene4Sound","./sounds/scene/scene4.wav",this.scene,null,{loop:true,autoplay:true});
    }
